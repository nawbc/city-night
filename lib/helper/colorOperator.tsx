export const hexToRgb = function (str: string) {
	...
	return (literal.length === 3) ? literal.match(/./g)!.map(val => parseInt(val, 16) ** 2) : literal.match(/../g)!.map(val => parseInt(val, 16));
};
...
export const makeColorDarker = function (color: string, level: number) {
	return 'rgb(' + color.slice(4, color.length - 1).split(',').map(v => Math.ceil(parseInt(v) * level)).join(',') + ')';
};

export const makeColorLighter = function (color: string, level: number) {
	return 'rgb(' + color.slice(4, color.length - 1).split(',').map(v => Math.ceil((255 - parseInt(v)) * level + parseInt(v))).join(',') + ')';
};
